...
def RFClassif(vectorFile, opathFcl, opathT, opathFim, *SerieList):
    ...
    Classif = "otbcli_TrainImagesClassifier -io.il " + dataSeries + " -io.vd " + vectorFile \
              + " -sample.bm " + str(
        bm) + " -io.confmatout " + newpath + "/RF_ConfMat_" + seed \
              + "_bm" + str(bm) + ".csv -io.out " + newpath + "/RF_Classification_" + seed + "_bm" + str(
        bm) + ".txt " + ch
    os.system(Classif)
    print Classif
...
def SVMClassif(vectorFile, opathFcl, opathT, opathFim, *SerieList):
    ...
    Classif = "otbcli_TrainImagesClassifier -io.il " + dataSeries + " -io.vd " + vectorFile \
              + " -sample.bm " + str(
        bm) + " -io.confmatout " + newpath + "/SVM_ConfMat_" + seed \
              + "_bm" + str(bm) + ".csv -io.out " + newpath + "/SVM_Classification_" + seed + "_bm" + str(
        bm) + ".txt " + ch
    os.system(Classif)
    print Classif
    ...
    for value in dico.random:
       for bm in bound:
          ...
          +" -rand "+str(value)+" -sample.bm "+str(bm)+" -io.confmatout "+newpath+"/SVM_ConfMat"+str(i)\
...
def imageClassification(model, image, opath, mask):
    ...
    confName = opath + "/" + modelName[-1] + "_confiance.tif"
    Classif = "otbcli_ImageClassifier -in " + image +" -model " + model \
...
def ConfMatrixFusion(classification, refdata, opath):
    ...
    classname = classPath[0].split('/')
    algoname = classname[-1].split('_')
    confName = opath + "/" + algoname[0] + algoname[3] + "_ConfMat_" + algoname[2] + "_" + algoname[4] + algoname[
        -1] + ".csv"
    Compute = "otbcli_ComputeConfusionMatrix -in " + classification + " -out " + confName \
              + " -ref vector -ref.vector.field CODE -ref.vector.in " + refdata
    print Compute
