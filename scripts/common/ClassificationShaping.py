...
def BuildConfidenceCmd(finalTile, classifTile, confidence, OutPutConfidence, fact=1):
    ...
    exp = []
    for i in range(len(classifTile)):
        exp.append("(im" + str(i + 2) + "b1==0?0:im1b1!=im" + str(i + 2) + "b1?1-im" + str(i + 2 + N) + "b1:im" + str(
            i + 2 + N) + "b1)")
    ...
    cmd = 'otbcli_BandMath -il ' + finalTile + ' ' + All + ' -out ' + OutPutConfidence + ' -exp "' + str(
        fact) + '*(' + expConfidence + ')"'
    return cmd
            ...
            if mode != 'outside':
                if classifMode == "separate":
                    confidence = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*model*confidence_seed_" + str(seed) + "*")
                    ...
                    finalTile = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*NODATA*_seed" + str(seed) + "*")  # final tile (without nodata)
                    classifTile = fu.fileSearchRegEx(pathToClassif + "/Classif_" + tuile + "*model*_seed_" + str(
                        seed) + "*")  # tmp tile (produce by each classifier, without nodata)
                    confidence = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*model*confidence_seed_" + str(seed) + "*")
                ...
                if classifMode != "separate":
                    classifTile = fu.fileSearchRegEx(pathToClassif + "/Classif_" + tuile + "*model_*f*_seed_" + str(
                        seed) + "*")  # tmp tile (produce by each classifier, without nodata)
                    splitModel = []
                    ...
                    for model in splitModel:
                        classifTile = fu.fileSearchRegEx(
                            pathToClassif + "/Classif_" + tuile + "*model_" + model + "f*_seed_" + str(
                                seed) + "*")  # tmp tile (produce by each classifier, without nodata)
                        finalTile = pathToClassif + "/Classif_" + tuile + "_model_" + model + "_seed_" + str(
                            seed) + ".tif"
                        confidence = fu.fileSearchRegEx(
                            pathToClassif + "/" + tuile + "*model_" + model + "f*_confidence_seed_" + str(seed) + "*")
                        classifTile = sorted(classifTile)
                        confidence = sorted(confidence)
                    ...
                    confidenceTMP = fu.fileSearchRegEx(
                        pathToClassif + "/" + tuile + "*model_*_confidence_seed_" + str(seed) + "*")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-path.classif",
                        help="path to the folder which ONLY contains classification images (mandatory)",
                        dest="pathClassif", required=True)
    parser.add_argument("-path.envelope",
                        help="path to the folder which contains tile's envelope (with priority) (mandatory)",
                        dest="pathEnvelope", required=True)
    parser.add_argument("-path.img", help="path to the folder which contains images (mandatory)", dest="pathImg",
                        required=True)
    ...
    parser.add_argument("-path.out",
                        help="path to the folder which will contains all final classifications (mandatory)",
                        dest="pathOut", required=True)
    ...
    ClassificationShaping(args.pathClassif, args.pathEnvelope, args.pathImg, args.fieldEnv, args.N, args.pathOut,
                          args.pathWd, args.pathConf, args.colorpath)
