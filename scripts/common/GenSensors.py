...
class Sensor(object):
    ...
    def ResizeMasks(self, opath, imref):
        ...
        if otbVersion == 5.0:
            expMask = {"NUA": "im1b1 and 00000001?1:im1b1 and 00001000?1:0", "SAT": "im1b1!=0",
                       "DIV": "im1b1 and 00000001?1:0"}
        else:
            expMask = {"NUA": "if(im1b1 and 00000001,1,if(im1b1 and 00001000,1,0))", "SAT": "im1b1!=0",
                       "DIV": "if(im1b1 and 00000001,1,0)"}
        ds = gdal.Open(imref, gdal.GA_ReadOnly)
        nb_col = ds.RasterXSize
        nb_lig = ds.RasterYSize
        proj = ds.GetProjection()
