...
def PreProcessS2(config, tileFolder, workingDirectory):
    ...
    for date in dates:
        ...
        for Ccloud, Csat, Cdiv in zip(AllCloud, AllSat, AllDiv):
            ...
            # Generate reproj even if no reprojection is needed (to improve!)
            outFolder = os.path.split(Ccloud)[0]
            cloudOut = os.path.split(Ccloud)[1].replace(".tif", "_reproj.tif")
            if int(cloudProj) != int(projOut):
            ...
            if int(divProj) != int(projOut):
                ...
                shutil.copy(Cdiv, outFolder + "/" + divOut)
    ...
    if binding == 'False':
        if batchProcessing == 'False':
            ...
            for sensor in list_Sensor:
                ...
                cmd = "otbcli_iota2FeatureExtraction -in " + serieTempGap + " -out " + outputFeatures + " int16 -comp " + comp + " -red " + red + " -nir " + nir + " -swir " + swir + " -copyinput true"
                print cmd
                deb = time.time()
                os.system(cmd)
                fin = time.time()
