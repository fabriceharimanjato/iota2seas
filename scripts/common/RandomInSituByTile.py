...
def RandomInSitu(vectorFile, field, nbdraws, opath, name, AllFields, ratio, pathWd):
    ...
    for tirage in range(0, nbtirage):
        ...
        if pathWd == None:
            ...
            fu.CreateNewLayer(layer, outShapefile, AllFields)
            fu.cpShapeFile(outShapefile.replace(".shp", ""), opath + "/" + name + "_seed" + str(tirage) + "_learn",
                           [".prj", ".shp", ".dbf", ".shx"])
        ...
        if pathWd == None:
            ...
            fu.CreateNewLayer(layer, outShapefile2, AllFields)
            fu.cpShapeFile(outShapefile2.replace(".shp", ""), opath + "/" + name + "_seed" + str(tirage) + "_val",
                           [".prj", ".shp", ".dbf", ".shx"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-shape.dataTile",
                        help="path to a shapeFile containing data's for one region in a tile (mandatory)", dest="path",
                        required=True)
    parser.add_argument("-shape.field", help="data's field into shapeFile (mandatory)", dest="dataField", required=True)
    parser.add_argument("--sample", dest="N", help="number of random sample (default = 1)", default=1, type=int,
                        required=False)
    ...
    parser.add_argument("-ratio", dest="ratio",
                        help="Training and validation sample ratio  (mandatory, default value is 0.5)", default='0.5',
                        required=True)
