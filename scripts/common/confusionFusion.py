...
def confFusion(shapeIn, dataField, csv_out, txt_out, csvPath, pathConf):
    ...
    for seed in range(N):
        ...
        writeResults(Fs, Rec, Pre, kappa, overallAccuracy, AllClass,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-path.shapeIn", help="path to the entire ground truth (mandatory)", dest="shapeIn",
                        required=True)
    parser.add_argument("-dataField", help="data's field inside the ground truth shape (mandatory)", dest="dataField",
                        required=True)
    ...
    parser.add_argument("-path.csv", help="where are stored all csv files by tiles (mandatory)", dest="csvPath",
                        required=True)
