...
def assembleTile_Merge(AllRaster, spatialResolution, out):
    ...
    cmd = "gdal_merge.py -ps " + str(spatialResolution) + " -" + str(
        spatialResolution) + " -o " + out + " -ot Int16 -n 0 " + AllRaster
    print cmd
    os.system(cmd)
...
def checkConfigParameters(pathConf):
    ...
    if shapeMode == "points":
        if ("-sample.mt" or "-sample.mv" or "-sample.bm" or "-sample.vtr") in options:
            error.append(
                "wrong options passing in classifier argument see otbcli_TrainVectorClassifier's documentation\n")
...
def ResizeImage(imgIn, imout, spx, spy, imref, proj, pixType):
    ...
    Resize = 'gdalwarp -of GTiff -tr ' + spx + ' ' + spy + ' -te ' + str(minX) + ' ' + str(minY) + ' ' + str(
        maxX) + ' ' + str(maxY) + ' -t_srs "EPSG:' + proj + '" ' + imgIn + ' ' + imout
    print Resize
    os.system(Resize)
