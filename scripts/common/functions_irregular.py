...
def RFClassif(vectorFile, opathFcl, opathFim, tile, *SerieList):
    ...
    Classif = "otbcli_TrainImagesClassifier -io.il " + dataSeries + " -io.vd " + vectorFile \
              + " -sample.bm " + str(
        bm) + " -io.confmatout " + newpath + "/RF_ConfMat_" + tile \
              + "_bm" + str(bm) + ".csv -io.out " + newpath + "/RF_Classification_" + tile + "_bm" + str(
        bm) + ".txt " + ch
    print Classif
    os.system(Classif)
...
def GetPixelValue(X, Y, image, band):
    values = os.popen(
        'gdallocationinfo ' + image + ' -valonly -geoloc -b ' + str(band) + ' ' + str(X) + ' ' + str(Y)).read()
    return values
