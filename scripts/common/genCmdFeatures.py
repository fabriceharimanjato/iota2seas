...
def CmdFeatures(testPath, tiles, appliPath, pathL8, pathL5, pathS2, pathConfig, pathout, pathWd):
    ...
    for i in range(len(tiles)):
        ...
        if pathWd == None:
            # Sequential mode
            Allcmd.append(
                "python " + appliPath + "/New_ProcessingChain.py " + gap + " -cf " + pathConfig + " -iL8 " + pathL8 + "/Landsat8_" +
                tiles[i] + " -iL5 " + pathL5 + "/Landsat5_" + tiles[i] + " -w " + pathout + "/" + tiles[
                    i] + " --db_L5 " + begDateL5 + " --de_L5 " + endDateL5 + " --db_L8 " + begDateL8 + " --de_L8 " + endDateL8 + " -wr " + wr + " -iS2 " + pathS2 + "/" +
                tiles[i] + " --db_S2 " + begDateS2 + " --de_S2 " + endDateS2)
        else:
            # HPC
            Allcmd.append(
                "python " + appliPath + "/processingFeat_hpc.py " + gap + " -cf " + pathConfig + " -iL8 " + pathL8 + "/Landsat8_" +
                tiles[i] + " -iL5 " + pathL5 + "/Landsat5_" + tiles[
                    i] + " -w $TMPDIR --db_L8 " + begDateL8 + " --de_L8 " + endDateL8 + " --db_L5 " + begDateL5 + " --de_L5 " + endDateL5 + " -wr " + wr + " --wo " + pathout + "/" +
                tiles[i] + " -iS2 " + pathS2 + "/" + tiles[
                    i] + " --db_S2 " + begDateS2 + " --de_S2 " + endDateS2 + " > " + logPath + "/" + tiles[
                    i] + "_feat.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-path.application", help="path to python's applications (mandatory)", dest="appliPath",
                        required=True)
    ...
    parser.add_argument("-path.config", help="path to the configuration file(mandatory)", dest="pathConfig",
                        required=True)
    parser.add_argument("-path.out", help="path out(mandatory)", dest="pathout", required=True)
    parser.add_argument("--wd", dest="pathWd", help="path to the working directory", default=None, required=False)
    args = parser.parse_args()

    CmdFeatures(args.testPath, args.tiles, args.appliPath, args.pathL8, args.pathL5, args.pathS2, args.pathConfig,
                args.pathout, args.pathWd)
