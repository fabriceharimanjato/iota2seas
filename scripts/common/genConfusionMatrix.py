...
def compareRef(shapeRef, shapeLearn, classif, diff, footprint, workingDirectory, pathConf):
    ...
    # Rasterise val
    cmd = "gdal_rasterize -a " + dataField + " -init 0 -tr " + str(spatialRes) + " " + str(
        spatialRes) + " " + shapeRef + " " + shapeRaster_val + " -te " + str(minX) + " " + str(minY) + " " + str(
        maxX) + " " + str(maxY)
    print cmd
    os.system(cmd)
    ...
    # Rasterise learn
    cmd = "gdal_rasterize -a " + dataField + " -init 0 -tr " + str(spatialRes) + " " + str(
        spatialRes) + " " + shapeLearn + " " + shapeRaster_learn + " -te " + str(minX) + " " + str(minY) + " " + str(
        maxX) + " " + str(maxY)
    print cmd
    os.system(cmd)
    ...
    for seed in range(N):
        ...
        for tile in AllTiles:
            ...
            cmd = 'otbcli_ComputeConfusionMatrix -in ' + pathClassif + '/Classif_Seed_' + str(
                seed) + '.tif -out ' + pathDirectory + '/' + tile + '_seed_' + str(
                seed) + '.csv -ref.vector.field ' + dataField + ' -ref vector -ref.vector.in ' + pathTMP + '/ShapeValidation_' + tile + '_seed_' + str(
                seed) + '.shp'
            AllCmd.append(cmd)
            ...
            compareRef(pathTMP + '/ShapeValidation_' + tile + '_seed_' + str(seed) + '.shp',
                       pathTMP + '/ShapeLearning_' + tile + '_seed_' + str(seed) + '.shp', classif, diff, footprint,
                       workingDirectory, pathConf)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-path.classif", help="path to the folder which contains classification images (mandatory)",
                        dest="pathClassif", required=True)
    parser.add_argument("-path.valid",
                        help="path to the folder which contains validation samples (with priority) (mandatory)",
                        dest="pathValid", required=True)
    ...
    parser.add_argument("-confusion.out.cmd", dest="pathToCmdConfusion",
                        help="path where all confusion cmd will be stored in a text file(mandatory)", required=True)
    ...
    genConfMatrix(args.pathClassif, args.pathValid, args.N, args.dataField, args.pathToCmdConfusion, args.pathConf,
                  args.pathWd)
