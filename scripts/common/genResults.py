...
def genResults(pathRes, pathNom):
    ...
    listClass, PreClass, RcallClass, FSClass, Kappa, OA = getCoeff(pathRes + "/TMP",
                                                                   pathNom)  # Récupération de toutes les valeurs
    ...
    for i in range(len(listClass)):
        ...
        binf, bSup = stats.t.interval(0.95, len(listClass) - 1, loc=np.mean(np.mean(PreClass[i])),
                                      scale=stats.sem(PreClass[i]))
        PreI.append("{:.4f}".format(float(np.mean(PreClass[i]) - binf)))
        binf, bSup = stats.t.interval(0.95, len(listClass) - 1, loc=np.mean(np.mean(RcallClass[i])),
                                      scale=stats.sem(RcallClass[i]))
        RecI.append("{:.4f}".format(float(np.mean(RcallClass[i]) - binf)))
        binf, bSup = stats.t.interval(0.95, len(listClass) - 1, loc=np.mean(np.mean(FSClass[i])),
    ...
    resfile.write("\n%s | %s | %s | %s | %s\n" % (
    CreateCell("Classes", sizeClass), CreateCell("Precision moyenne", sizePre), CreateCell("Rappel moyen", sizeRec),
    CreateCell("F-score moyen", sizeFS), CreateCell("Confusion max", sizeConf)))
    resfile.write("%s\n" % (sep))
    for i in range(len(listClass)):
        resfile.write("%s | %s | %s | %s | %s\n" % (
        CreateCell(listClass[i], sizeClass), CreateCell(Pre_S[i], sizePre), CreateCell(Rec_S[i], sizeRec),
...
if __name__ == "__main__":
    ...
    parser.add_argument("-path.res", help="path to the folder which contains classification's results (mandatory)",
                        dest="pathRes", required=True)
    parser.add_argument("-path.nomenclature	", help="path to the nomenclature (mandatory)", dest="pathNom",
                        required=True)
