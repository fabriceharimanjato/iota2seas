	...
	<div class = "row">\n\
        ...
        for i in range(len(lineData)):
            if lineData[i][1] != "autres":
                ...
                lineData[i][2], lineData[i][3], lineData[i][4], lineData[i][0], lineData[i][1], lineData[i][5]))
            else:
                """
...
if __name__ == "__main__":
    if len(sys.argv) < 6:
        print "Usage: " + sys.argv[
            0] + " server_url style_file output_path classif_1 metrics_1 [classif_12 metrics_2 ... classif_n metrics_n]"
        print "Example: python " + sys.argv[
            0] + " \"http://cyan.ups-tlse.fr:8080/geoserver/SudOuest/wms?\" FR_ALLCLASSES.qml /tmp/html \"SudOuest:OSOV1\" MetricsV1.txt \"SudOuest:OSOV2\" MetricsV2.txt "
    else:
