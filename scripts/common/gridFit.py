...
def getAllDatasByDate(currentTile, rasterPatterns, masksPatterns, arboRaster, arboMask, TileFolder, rasterInitValue,
                      masksInitValues):
...
def launchFit(sensorName, cmdPath, workingDirectory, outDirectory, outGridPath, outputGrid_tileNameField, outPixRes,
              outputProjection, interpolator, inputGrid_tileNameField, inputGRID, arboMask, masksInitValues,
              masksPatterns, arboRaster, rasterInitValue, rasterPatterns, TileFolder):
    ...
    for refTile, tiles in intersections:
        ...
        for currentTile in tiles:
            datas, dates = getAllDatasByDate(currentTile, rasterPatterns, masksPatterns, arboRaster, arboMask,
                                             TileFolder, rasterInitValue, masksInitValues)
            for currentDatas, currentDate in zip(datas, dates):
                ...
                for currentRaster in currentDatas:
                    ...
                    outName = currentRaster[0].split("/")[-1].replace(currentTile, refTile).replace(".tif",
                                                                                                    "_" + currentTile + ".tif")
                    out = folder + "/" + outName
...
if __name__ == "__main__":
    ...
    launchFit(args.sensorName, args.cmdPath, args.workingDirectory, args.outDirectory, args.outGridPath,
              args.outputGrid_tileNameField, args.outPixRes, args.outputProjection, args.interpolator,
              args.inputGrid_tileNameField, args.inputGRID, args.arboMask, args.masksInitValues, args.masksPatterns,
              args.arboRaster, args.rasterInitValue, args.rasterPatterns, args.TileFolder)
