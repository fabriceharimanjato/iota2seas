...
class LogPreprocess(Log):
    ...
    def compareLogInstanceArgs(self, log_old):
        # A changer si ajout d'étape
        same = True
        if not ((log_old.ipathL5 == self.ipathL5) or (log_old.work_res == self.work_res) or (
            log_old.ipathF == self.ipathF) or (log_old.ipathL8 == self.ipathL8) or (log_old.ipathS4 == self.ipathS4)):
