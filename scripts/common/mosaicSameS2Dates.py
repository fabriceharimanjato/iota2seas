    ...
    for currentSameDate in sameDateFolder:
        ...
        for currentBand in S2Bands:
            ...
            for currentFolder in currentSameDate:
                ...
                if not t in AllTiles: AllTiles.append(t)

            outFolder = tileFolder + "/" + currentDestTile + "/" + sensorName + "_" + currentDestTile + "_" + currentDate + "_" + "_".join(
                AllTiles)
            initVal = noDataR
            ...
            if not os.path.exists(outFolder + "/" + outName):
                ...
                if workingDirectory: workingFolder = workingDirectory
                cmd = "gdal_merge.py -init " + str(initVal) + " -n " + str(
                    initVal) + " -o " + workingFolder + "/" + outName + " " + priorityPaths
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tiles.folder", dest="tileFolder", help="path to the folder which contain all tile's folder",
                        default=None, required=True)
    parser.add_argument("-current.tile", dest="currentTile", help="tile to be process (must refers to a folder)",
                        default=None, required=True)
    parser.add_argument("-sensor.name", dest="sensorName", help="", default=None, required=True)
    parser.add_argument("-raw.folder", dest="S2Folder", help="folder which raw datas, before any process", default=None,
                        required=True)
    parser.add_argument("-raster.bands", nargs='+', dest="S2Bands", help="all bands to be process without data's mask",
                        default=None, required=True)
    parser.add_argument("-raster.masks", nargs='+', dest="masks", help="data's mask", default=None, required=True)
    parser.add_argument("-bands.noData", dest="noDataR", help="no data's value in bands raster", default=None,
                        required=True)
    ...
    parser.add_argument("-date.position", dest="rasterPosDate",
                        help="date position in raster name if split by '_' and start counting by 0", type=int,
                        default=None, required=True)

    args = parser.parse_args()

    launchFit(args.noDataM, args.noDataR, args.tileFolder, args.currentTile, args.sensorName, args.S2Folder,
              args.S2Bands, args.masks)
