...
def gen_MaskRegionByTile(fieldRegion, Stack_ind, workingDir, currentTile, AllModel, shpRName, pathToImg, pathTest,
                         pathWd, pathToConfig):
    modelTile = []
...
def concatClassifs_OneTile(pathWd, seed, currentTile, pathTest, modelTile, concatOut):
    ...
    for model in modelTile:
        classifFusion.append(
            pathTest + "/classif/Classif_" + currentTile + "_model_" + model + "_seed_" + str(seed) + ".tif")
    ...
    if len(classifFusion) == 1:
        ...
        cmd = "otbcli_ConcatenateImages -ram 128 -il " + stringClFus + " -out " + pathToDirectory + "/" + \
              concatOut.split("/")[-1]
        print cmd
        os.system(cmd)
...
def concatRegion_OneTile(currentTile, pathTest, classifFusion_mask, pathWd, TileMask_concat):
    ...
    if len(classifFusion_mask) == 1 and not os.path.exists(TileMask_concat):
        ...
        classifFusion_MASK_sort = sorted(classifFusion_mask,
                                         key=getModelinMASK)  # in order to match images and their mask
        stringClFus = " ".join(classifFusion_MASK_sort)

        pathDirectory = pathTest + "/classif"
        if pathWd != None:
            pathDirectory = pathWd
        cmd = "otbcli_ConcatenateImages -ram 128 -il " + stringClFus + " -out " + pathDirectory + "/" + \
              TileMask_concat.split("/")[-1]
        print cmd
        os.system(cmd)
...
def noData(pathTest, pathFusion, fieldRegion, pathToImg, pathToRegion, N, pathConf, pathWd):
    ...
    if modeClassif != "outside":
        modelTile = gen_MaskRegionByTile(fieldRegion, Stack_ind, workingDir, currentTile, AllModel, shpRName, pathToImg,
                                         pathTest, pathWd, outputPath + "/config_model/configModel.cfg")
    ...
    if len(modelTile) == 0 or noLabelManagement == "maxConfidence":
        for seed in range(N):
            imgConfidence = fu.FileSearch_AND(pathTest + "/classif", True, "confidence_seed_" + str(seed) + ".tif",
                                              currentTile)
            ...
            if modeClassif == "outside":
                imgConfidence = fu.fileSearchRegEx(
                    pathTest + "/classif/" + currentTile + "_model_" + modelTile + "f*_confidence_seed_" + str(
                        seed) + ".tif")
                imgClassif = fu.fileSearchRegEx(
                    pathTest + "/classif/Classif_" + currentTile + "_model_" + modelTile + "f*_seed_" + str(
                        seed) + ".tif")
                imgData = pathDirectory + "/Classif_" + currentTile + "_model_" + modelTile + "_seed_" + str(
                    seed) + ".tif"
            imgConfidence.sort()
            imgClassif.sort()
        ...
        for seed in range(N):
            ...
            if modeClassif == "outside":
                concatOut = pathTest + "/classif/" + currentTile + "_FUSION_model_" + modelTile[0].split("f")[
                    0] + "concat_seed" + str(seed) + ".tif"
            ...
            if modeClassif == "outside":
                ...
                outConcat_mask = pathTest + "/classif/" + currentTile + "_MASK_model_" + modelTile[0].split("f")[
                    0] + ".tif"
                classifFusion_mask = []
                for i in range(Nfold):
            ...
            imgData = pathDirectory + "/" + currentTile + "_FUSION_NODATA_seed" + str(seed) + ".tif"
            if modeClassif == "outside":
                imgData = pathDirectory + "/Classif_" + currentTile + "_model_" + modelTile[0].split("f")[
                    0] + "_seed_" + str(seed) + ".tif"
...
if __name__ == "__main__":
    ...
    parser.add_argument("-tile.fusion.path", help="path to the classification's images (with fusion)",
                        dest="pathFusion", required=True)
    ...
    noData(args.pathTest, args.pathFusion, args.fieldRegion, args.pathToImg, args.pathToRegion, args.N, args.pathConf,
           args.pathWd)
