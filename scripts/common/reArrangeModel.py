...
def generateRepartition(pathTest, config, rep_model, rep_model_repCore, dataField):
    ...
    for seed in range(N):
        ...
        for model_cor, tiles_cor in repCore:
            for tile_cor in tiles_cor:
                if not os.path.exists(shapeApp + "/" + tile + "_region_" + str(model_cor) + "_seed" + str(
                        seed) + "_learn.shp"):
                    learnShp = fu.FileSearch_AND(shapeApp, True, tile, "seed" + str(seed) + "_learn.shp")
                    cmd1 = "cp " + learnShp[0] + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_learn.shp"
                    cmd2 = "cp " + learnShp[0].replace(".shp", ".shx") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_learn.shx"
                    cmd3 = "cp " + learnShp[0].replace(".shp", ".dbf") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_learn.dbf"
                    cmd4 = "cp " + learnShp[0].replace(".shp", ".prj") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_learn.prj"

                    cmd5 = "cp " + learnShp[0].replace("_learn.shp",
                                                       "_val.shx") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_val.shp"
                    cmd6 = "cp " + learnShp[0].replace("_learn.shp",
                                                       "_val.shx") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_val.shx"
                    cmd7 = "cp " + learnShp[0].replace("_learn.shp",
                                                       "_val.dbf") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_val.dbf"
                    cmd8 = "cp " + learnShp[0].replace("_learn.shp",
                                                       "_val.prj") + " " + shapeApp + "/" + tile + "_region_" + str(
                        model_cor) + "_seed" + str(seed) + "_val.prj"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="This function try to rearrange the repartition tile by model, considering class repartition")
    ...
    generateRepartition(args.pathTest, args.config, args.rep_model, args.rep_model_repCore, args.dataField)
