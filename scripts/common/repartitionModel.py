...
def getDiag(listTiles, Tile):
    ...
    for currentTile in listTiles:
        ...
        if (currentX == X - 1 and (currentY == Y - 1 or currentY == Y + 1)) or (
                currentX == X + 1 and (currentY == Y - 1 or currentY == Y + 1)):
...
def get4voisins(listTiles, Tile):
    ...
    for currentTile in listTiles:
        currentX = currentTile.getX()
        currentY = currentTile.getY()
        if (currentX == X and (currentY == Y - 1 or currentY == Y + 1)) or (
                currentY == Y and (currentX == X - 1 or currentX == X + 1)):
            v.append(currentTile)
    return v
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model.delta", default=None, type=int, dest="delta",
                        help="difference between the most reprensented model and the less", required=False)
