...
def generateRegionShape(mode, pathTiles, pathToModel, pathOut, fieldOut, pathConf, pathWd):
    ...
    if mode == "one_region":
        ...
        if not pathToModel: raise Exception(
...
if __name__ == "__main__":
    ...
    parser.add_argument("-mode", dest="mode", help="one_region/multi_regions (mandatory)",
                        choices=['one_region', 'multi_regions'], required=True)
    parser.add_argument("-fieldOut", dest="fieldOut", help="field out (mandatory)", required=True)
    parser.add_argument("-pathTiles", dest="pathTiles", help="path where are only stored tile's envelope (mandatory)",
                        default="None", required=True)
    parser.add_argument("--multi.models", dest="pathToModel", help="path to the text file which link tiles/models",
                        default="None", required=False)
    parser.add_argument("-out", dest="pathOut", help="path where to store all shape by tiles (mandatory)",
                        default="None", required=True)
    ...
    generateRegionShape(args.mode, args.pathTiles, args.pathToModel, args.pathOut, args.fieldOut, args.pathConf,
                        args.pathWd)
