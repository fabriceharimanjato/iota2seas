...
def genTileEnvPrio(ObjListTile, out, tmpFile, proj):
    ...
    ObjListTile.reverse()
    listSHP = [createRasterFootprint(c_ObjListTile.getPath(), tmpFile + "/" + c_ObjListTile.getName() + ".shp") for
               c_ObjListTile in ObjListTile]
    ...
    for i in range(len(ObjListTile)):
        ...
        for j in range(1 + i, len(ObjListTile)):
            ...
            if IsIntersect(currentTileEnv, NextTileEnv):
                ...
                intersection = fu.ClipVectorData(ObjListTile[i].getEnvelope(), ObjListTile[j].getEnvelope(), tmpFile,
                                                 InterName)
        ...
        for j in range(1 + i, len(ObjListTile)):
            ...
            if IsIntersect(currentTileEnv, NextTileEnv):
                if diag(ObjListTile[i], ObjListTile[j]):
                    ...
                    tmpName = ObjListTile[i].getName() + "_TMP"
                    subtractShape(ObjListTile[i].getPriorityEnv(), ObjListTile[j].getPriorityEnv(), tmpFile, tmpName,
                                  proj)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="This function allow you to generate tile's envelope considering tile's priority")
